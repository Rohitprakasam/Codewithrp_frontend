      ...
      <main className="flex-1 flex flex-col min-w-0">
        ...
        <div className="p-6 lg:p-8 space-y-8">
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            ...
            <StatCard icon={Users} label="Active students" value={48} color="text-warning" />
            <StatCard icon={CheckCircle2} label="Acceptance rate" value="62%" color="text-success" />
          </div>
                  ...
                  {p ? (
                    <>
                      <div className="flex items-center gap-3 min-w-0">
                        <span className="text-muted-foreground tabular-nums w-6">{(p as Problem).id}</span>
      ...
      >
        ...
        <div className="p-5 space-y-4">
          <div className="grid grid-cols-2 gap-4 text-sm">
            <Info label="Student" value={sub.user} />
            <Info label="Problem" value={sub.problemTitle} />
            <Info label="Language" value={sub.language} />
            <Info label="Runtime" value={sub.runtime} />
            <Info label="Status" value={<StatusBadge value={sub.status} />} />
            <Info label="Submitted" value={sub.submittedAt} />
          </div>
          <div>
            <div className="text-xs text-muted-foreground mb-1.5">Code</div>
            <pre className="font-mono text-xs whitespace-pre-wrap rounded-md bg-[color:var(--editor)] border border-border p-4 max-h-64 overflow-auto scrollbar-thin">{`function solution(input) {\n  // submitted code preview\n  return input;\n}`}</pre>
          </div>
        </div>
      </motion.div>
    </motion.div>
